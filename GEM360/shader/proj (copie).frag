...
vec4 colSel = vec4(1.0,0.5,0.0,0.7);
...
{
  vec3 rc = ray;//-center;
  float c = dot(rc, rc) - 1;//(radius*radius);
  float b = dot(dir, rc);
  float d = b*b - c;
...
void main( void ) {
    ...
    //vec2 texuv = controlToTex(screenToControl(uv*-1));
    vec2 texuv = screenToTex(uv*-1);
    
    //vec3 d = camdir + /*rotationMatrix**/vec3(camfov*texuv, 0.0);
