...
t_PID SpeedPID_B; //speed pid motor C
//volatile DWORD LastCapture4={0,0,0,0};
volatile unsigned long LastIncTimeB;
volatile unsigned long int DeltaTB; // interval between incs
volatile long int PosB;
volatile char DirectionB=1; //1=incr -1=decr 
int SpeedConsignB,PWMConsignB;
int ModeB ; //0:pwm 1:speed 2:pos 
...
#define MAXSPEED ((unsigned long)(1UL<<22))
	...
	if(ModeB==1) {
		error=((long)(SpeedConsignB)-(long)actualSpeed)/*>>4*/;
	
		if(error>=(1L<<15)) error=(1L<<15)-1;
		else if(error<-(1L<<15)) error=-(1L<<15);
		PIDCompute(&SpeedPID_B,(int)error);
	...
	if(v<-1023) v=-1023;
	//if(v<0) v=0; // one way
	v=(MotorB.Status.reversed?-v:v);
	
	vabs=v<0?1023+v:v;
	SET_PWM(5,vabs);
	
	if(v<0) { MOTB_IN1=1 ; /*MOTB_IN2=0 ;*/ }
	else { MOTB_IN1=0 ; /*MOTB_IN2=1 ; */}	
}
void CaptureService()
{
...
/*void CaptureService__()
{
	static unsigned long Capture,d;
	
	if(!PIR4bits.CCP4IF) return;	
			
	Capture=CCPR4L|(CCPR4H<<8)|(Now&0xFF00);

	
	if(Now<Capture) {
		Capture-=0x100;
	}
	d=(Capture._dword-LastCapture4._dword);//&0xFFFFFF;
	DeltaT+=d;
	LastCapture4._dword=Capture._dword;
	ICount++; ILastInc=1;
	PIR4bits.CCP4IF=0;
}*/
...
void HighInterrupts()
{
	...
	if(PIR2bits.TMR3IF) {
		...
		if(StroboMode1>1) MOTA_IN1=s;
		if(StroboMode2>1) MOTA_IN2=!s;
...
void main(void)
{
	...
	//SETPORT_MOTC;
	//SETPORT_MOTD;
	SetPinDigiIn(MOTB_INC);	SetPinAnsel(MOTB_INC,0);
...
// --------- init motors PWM on TMR2 -----------
	CCP5CON=CCP1CON=0b00001100 ; // single PWM active high
	//CCP2CON=
...
void Input()
{
	...
	switch(c) {
		...
		PARAM_CHAR(4,StroboMode1); if(StroboMode1<2) MOTA_IN1=(StroboMode1!=0); break;
		PARAM_CHAR(3,StroboMode2); if(StroboMode2<2) MOTA_IN2=(StroboMode2!=0); break;
		PARAM_INT(5,StroboTime); if(StroboTime<STROBOTIMEMIN) StroboTime=STROBOTIMEMIN; break;
		PARAM_INT(6,i); SET_PWM(1,i); break;
		...
		case 110 : PIDInput(&SpeedPID_B,110);break;
		PARAM_INT(111,SpeedConsignB); ModeB=1;break;
		PARAM_INT(112,PWMConsignB); ModeB=0;break;
...
void EEdeclareMain()
{
	...
	DCM_declareEE(&MotorB);
	PID_declareEE(&SpeedPID_B);
}
